  ...
  return (
    <div className="summary">
      ...
      <div className="summary-actions">
        <button className="btn btn-primary btn-block" onClick={onDone}><RotateCcw size={14} /> Done / New Reasoning Round</button>
      </div>
    </div>
  );
}
...
function App() {
  ...
  function exportTrigonometryData() {
    ...
    if (topicQuestions.length === 0) {
      notify("No questions in this topic to export");
      return;
    }
  ...
  function startTest() {
    ...
    const count = Math.max(1, Math.min(Number(testConfig.count) || 1, max));

    // Create a fresh random order every time the user starts a test.
    // The original question-bank order is not changed.
    const randomizedQuestions = shuffleArr(pool);
    const ids = randomizedQuestions.slice(0, count).map((q) => q.id);
  ...
  return (
    <div className="app">
      ...
      <div className="header">
        <div className="header-title"><span className="brand">REASONING</span><span className="brand-accent">::BANK</span></div>
        <div className="header-actions">
          <button className="icon-btn export-btn" onClick={exportTrigonometryData} title="Download topic data" aria-label="Download topic data"><Icon symbol="v" size={16} /></button>
...
.app {
  --bg: #1d1b2a; --panel: #2c2942; --panel-2: #3c3858; --border: #5b5678;
  --panel: #121826;
  --panel-2: #171F30;
  --border: #232C40;
  --cyan: #7ad9d2; --green: #9adf8d; --rose: #f59a8b; --amber: #f5c66a;
  --violet: #4C8DFF;
  --green: #34C77B;
  --rose: #F2555B;
  --amber: #D9A441;
  --text: #f5f0ff; --muted: #d6cfe8;
  --muted: #8592A6;
...
.add-btn:hover { background: linear-gradient(135deg, #f3c36a, #e58f4d); box-shadow: 0 10px 22px rgba(245,198,106,.45); }
...
.section-head h2 { color: var(--text); }.count-pill { background: #4d5f7d; color: var(--cyan); }
.count-pill {
...
.badge.cat { color: #c6f3ff; background: #2d4d63; }.badge.ok { color: #dff7b0; background: #3d5e3d; }.badge.bad { color: #ffd1c4; background: #693f3f; }.badge.hide { color: #ffe3a1; background: #5e4a2c; }
...
.question-toggle:hover, .question-toggle:focus { background: #3e3b5c; }
...
.test-opts .opt-row.selectable:hover:not(:disabled) { border-color: var(--cyan); background: #2d4d63; color: var(--text); }.opt-row.opt-picked { background: #2d4d63; border-color: var(--cyan); }.opt-row.opt-correct-answer { background: #3b5a3e; border-color: var(--green); }.opt-row.opt-wrong-answer { background: #693f3f; border-color: var(--rose); }
...
.header { background: rgba(29,27,42,.97); border-color: var(--border); }
.header-title { font-family: 'Space Grotesk', sans-serif; font-size: 22px; letter-spacing: -.04em; }
.brand-accent { color: var(--cyan); }
.add-btn { background: linear-gradient(135deg, var(--amber), #ef9d5b); color: #1d1b2a; box-shadow: 0 8px 20px rgba(245,198,106,.35); }
.add-btn:hover { transform: rotate(5deg); background: #d19a43; }
.header-stats { background: rgba(44,41,66,.98); border-color: var(--border); }
.stat { background: #423d5f; border-color: var(--border); color: var(--muted); }
...
.empty-state { border-color: #7a7396; background: #27263a; color: var(--muted); }
.card { background: var(--panel); border-color: var(--border); box-shadow: 0 10px 24px rgba(12,10,18,.28); }.card:hover { box-shadow: 0 14px 30px rgba(12,10,18,.38); }
...
.link-btn { color: var(--cyan); }.solution { background: #2a2940; border-color: #6b6788; color: var(--text); }
...
.navbar { background: rgba(44,41,66,.98); border-color: var(--border); }.nav-btn { color: var(--muted); }.nav-btn.active { color: var(--amber); }.nav-btn.active::before { background: var(--amber); }
.toast { background: #141421; color: var(--text); border: 1px solid var(--border); }
.field-label { color: #e5d9ff; }.input, .textarea { background: #423d5f; border-color: var(--border); color: var(--text); }.input:focus, .textarea:focus { border-color: var(--cyan); box-shadow: 0 0 0 3px rgba(122,217,210,.2); }
.radio-dot { background: var(--panel-2); border-color: var(--border); color: var(--muted); }.radio-dot.checked { background: var(--green); color: #1d1b2a; border-color: var(--green); }.imgdrop { border-color: var(--border); color: var(--muted); }.imgdrop:hover { border-color: var(--cyan); color: var(--cyan); }
.btn { background: var(--panel-2); border-color: var(--border); color: var(--text); }.btn-primary { background: linear-gradient(135deg, var(--amber), #ef9d5b); border-color: transparent; color: #1d1b2a; box-shadow: none; }.btn-primary:hover:not(:disabled) { background: linear-gradient(135deg, #f4c676, #e68d4d); }.btn-ghost { background: transparent; }.btn-ok-active { color: var(--green); border-color: var(--green); background: #334b3b; }.btn-bad-active { color: var(--rose); border-color: var(--rose); background: #603b40; }
.setup-card { background: var(--panel); border-color: var(--border); box-shadow: 0 12px 28px rgba(12,10,18,.26); }.progress-track { background: #4e4b69; }.progress-fill { background: linear-gradient(90deg, var(--cyan), var(--amber)); }.q-card .q-text { color: var(--text); }
...
.modal-overlay { background: rgba(17,15,26,.72); }.modal { background: var(--panel); border-color: var(--border); }.modal-head, .modal-foot { border-color: var(--border); }
...
.opt-row { background: var(--panel-2); border-color: var(--border); color: var(--text); }.opt-row.opt-correct { background: #3b5a3e; border-color: var(--green); }.opt-letter { background: #5b5575; color: var(--cyan); }
...
.toggle-btn { background: var(--panel-2); border-color: var(--border); color: var(--muted); }.toggle-btn.active { color: var(--text); border-color: var(--cyan); background: #2d4d63; }
...
.topic-filter { display: flex; align-items: center; gap: 10px; margin: -4px 0 16px; padding: 10px 12px; border: 1px solid var(--border); border-radius: 12px; background: #2a2940; }.topic-filter .field-label { margin: 0; white-space: nowrap; }.topic-select { max-width: 240px; padding: 8px 10px; cursor: pointer; }.topic-select option { background: #2c2942; color: #f5f0ff; }
...
.solution a { color: var(--cyan); text-decoration: underline; overflow-wrap: anywhere; }
