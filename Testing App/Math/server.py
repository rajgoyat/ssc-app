"""Local server for the Math Bank app.
...
def load_questions():
    ...
    for file_path in [DATA_FILE] + sorted(TOPICS_DIR.glob("*.json")):
        for question in load_questions_from_file(file_path):
            if not isinstance(question, dict):
                continue
...
if __name__ == "__main__":
    ...
    print("Math Bank is running at http://localhost:8000")
    try:
